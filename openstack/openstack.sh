...
docker-machine create \
    ...
    --openstack-net-name cloudingair-6 \
    --openstack-floatingip-pool external-network \
    --openstack-ssh-user ubuntu \
    --openstack-sec-groups default,lab1 \
    --openstack-domain-name Default \
    cloudingair-6-machine
...
eval $(docker-machine env cloudingair-6-machine)

#Inciamos el modo docker swarm
docker swarm init

#Creamos la red overlay
docker network create -d overlay --attachable cloudingair-6-network
